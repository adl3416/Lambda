    ...
    public static void onbestenBykKckTekSayi(List<Integer> list){
       // list.stream().filter(t-> t%2==1).filter(t->t>15).reduce(Integer::min);
        ...
        System.out.println(list.
                stream(). //akisa girdi
                filter(t -> t % 2 == 1 & t > 15).  //tek ve 15den buyuk sart
                reduce(Integer::min)); //Optional[65]    min  deger reduce edildi

    }

    //list in cift elemanlarini kareleri ni  kucukten  buyu ge yazdiriniz

    public static void ciftKareKckByg(List<Integer>list){
        list.stream().
                filter(Lambda01::ciftBul).
                map(t->t*t). //kare almak gibi islem varsa map kullanilir
                sorted().   // bu method siraliyor
