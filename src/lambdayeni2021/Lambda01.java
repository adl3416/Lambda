...
// karekoklerini ayni satirda aralarina bosluk birakarak yazdiriniz
    ...
    public static void karekokCiftfunctional(List<Integer> list){
        list.stream().filter(Lambda01::ciftBul).map(Math::sqrt).forEach(t->System.out.println(t+" "));
                   //veya    (t->t%2==0)           (Math::sqrt) siralama yapar
    }

    //listin enbuyuk elemanini yazdirirmisiniz

    public static void maxElfunction(List<Integer>list){ //reduce() azaltma methodu: sarta gore azaltir
      Optional<Integer> maxEl= list.stream().reduce(Math::max); // reduce buyuk listeyi azaltti ve yeni maxEl tanimladik azaltilan elemani buraya cakti
        System.out.println(maxEl);
